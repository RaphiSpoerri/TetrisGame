const BRACKET = function() {
	...
	this.stages = [
		() => {
			...
			PIXELS[0].position = [mode, old];
			PIXELS[2].position = [6, old+1];
			PIXELS[1].position = [5, old+1];
			PIXELS[3].position = [4, old+1];
		},
		() => {
			var old = Math.min(...PIXELS.map(x=>x.y));
			...
			PIXELS[3].position = [6, old + mode - 4];
		},
		function() {
            const old_x = PIXELS[3].x;
			var old = Math.min(...PIXELS.map(x=>x.y));
			PIXELS[0].position = [4, old];
			PIXELS[1].position = [5, old];
			PIXELS[2].position = [6, old];
			PIXELS[3].position = [mode == 4 ? 6 : 4, old + 1];
		},
		function() {
			var old = Math.min(...PIXELS.map(x=>x.y));
			PIXELS[0].position = [5, old + (mode == 4 ? 6 : 4) - 4];
